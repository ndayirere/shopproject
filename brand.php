...
if(isset($_GET["bid"])){
  $id = $_GET["bid"];
  $sql = "SELECT categories.cat_title,brands.brand_title FROM categories INNER JOIN cat_brand ON cat_brand.cat_id = categories.cat_id INNER JOIN brands ON brands.brand_id = cat_brand.brand_id WHERE brands.brand_id = '$id'";
      ...
      <main class="osh-container" style="height:1400px;">
         <nav class="osh-breadcrumb">
            <ul>
               ...
               <li class="last-child" style="margin-top:-4px;"><a href="#" style="font-size:13px;"><?php echo $cat ?></a> <i style="font-size:13px;margin-top:-4px;margin-left:5px;" class="fa fa-angle-right"></i></li>
               <li class="last-child" style="margin-top:-4px;"><a href="#" style="font-size:13px;"><?php echo $brand ?></a></li>
            </ul>
         </nav>
                  ...
                  <div class="section group">
                    <div class="col_main span_main_01">
                      ...
                      <ul class="catnav">
                        ...
                        if(isset($_GET["bid"])){
                          $idb = $_GET["bid"];
                          $sql = "SELECT brands.* FROM brands INNER JOIN cat_brand ON cat_brand.brand_id = brands.brand_id INNER JOIN categories ON categories.cat_id = cat_brand.cat_id";
                          $run_query = mysqli_query($con, $sql);
                          while($row = mysqli_fetch_array($run_query)){
                            $bid = $row["brand_id"];
            ...
            <section class="osh-filters -vertical ft-vertical-filters">
               <section class="osh-category-tree">
                 ...
                 <ul class="osh-subcategories">
                   ...
                   if(isset($_GET["bid"])){
                     $idp = $_GET["bid"];
                     $sql = "SELECT COUNT(*),brands.brand_title FROM brands INNER JOIN products ON products.product_brand = brands.brand_id INNER JOIN cat_brand ON cat_brand.brand_id = brands.brand_id INNER JOIN categories ON categories.cat_id = cat_brand.cat_id GROUP BY brands.brand_title";
                     $run_query = mysqli_query($con, $sql);
                     while($row = mysqli_fetch_array($run_query)){
                       $bid = $row["brand_id"];
                       $brand_name = $row["brand_title"];
         ...
         <section class="osh-content" style="margin-top:-1600px;">
